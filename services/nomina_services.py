...
async def fetch_trabajadores():
    async with httpx.AsyncClient() as client:
        response = await client.post(f"{settings.API_BASE_URL}/nomina/trabajadores", json=app_state.conexion_params.dict())
        response.raise_for_status()
        return response.json()

async def fetch_relaciones_trabajadores():
    async with httpx.AsyncClient() as client:
        response = await client.post(f"{settings.API_BASE_URL}/nomina/relaciones-trabajadores", json=app_state.conexion_params.dict())
        response.raise_for_status()
        return response.json()
