...
def connection_form(store:AppState):
    ...
    with ui.column().classes('w-full h-screen flex items-center justify-center bg-gray-100 p-4'):
        with ui.card().classes('w-full max-w-md p-8 shadow-2xl'):
            ...
            password_input = ui.input('Contraseña', password=True, password_toggle_button=True).classes('w-full mb-4')
            error_label = ui.label('').classes('text-red-600 text-sm mb-4')

            async def connect():
                if not ip_input.value or not password_input.value or not database_input.value:
                    error_label.text = 'Todos los campos son requeridos'
                    return

                spinner = ui.spinner(size="lg", color="primary")
                spinner.classes('fixed top-0 left-0 right-0 bottom-0 m-auto z-50')  # centrar el spinner en pantalla
